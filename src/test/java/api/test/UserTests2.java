...
import api.endpoints.UserEndPointUingpropertifile;
...
public class UserTests2 {
   ...
   public void testPostUser(){
	   Response res=UserEndPointUingpropertifile.creatUser(userpayload);
	   res.then().log().all();
	   Assert.assertEquals(res.getStatusCode(),200);
   }
	@Test(priority=2)
	   public void testgetUser(){
		Response res=  UserEndPointUingpropertifile.readUser(this.userpayload.getUsername());
		   res.then().log().all();
		   Assert.assertEquals(res.getStatusCode(),200);
	   }
	...
	@Test(priority=3)
	   public void testUpdateUser(){
		...
		//upadte payload
		 ...
		 userpayload.setFirstName(fak.name().firstName());
		 userpayload.setEmail(fak.internet().safeEmailAddress());
		Response res=  UserEndPointUingpropertifile.updateUser(this.userpayload.getUsername(),userpayload);
		   res.then().log().all();
		   Assert.assertEquals(res.getStatusCode(),200);
		   //checking data after update
			Response resafterupdate=  UserEndPointUingpropertifile.readUser(this.userpayload.getUsername());
			   res.then().log().all();
	...
	@Test(priority=4)
	   public void testDeleteUser(){
		...
		Response res=  UserEndPointUingpropertifile.deleteUser(this.userpayload.getUsername());
		   Assert.assertEquals(res.getStatusCode(),200);
	   }
}
